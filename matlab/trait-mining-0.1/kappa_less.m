...
%    disp(tr)
%    kcomp;
    [k, po] = kcomp; % DE add output, 2010-08-15
%    disp(' ')
end
if w==1 || w==-1
    J=repmat((1:1:m),m,1);
...
%function kcomp % DE 2010-08-15 add output
function [k, po] = kcomp
global m f x alpha
...
%display results
% -- Moved Kappa coefficient up some steps (DTE, 2010-07-30)
% fprintf('Cohen''s kappa (k)       = %0.4f  <--- Kappa\n',k)
% fprintf('Cohen''s kappa = %0.4f\n',k)
% --
% fprintf('Observed agreement (po) = %0.4f  <--- PO\n',po)
...
%if k<0
%    disp('Poor agreement (N)')
%elseif k>=0 && k<=0.2
%    disp('Slight agreement (N)')
%elseif k>=0.21 && k<=0.4
%    disp('Fair agreement (*)')
%elseif k>=0.41 && k<=0.6

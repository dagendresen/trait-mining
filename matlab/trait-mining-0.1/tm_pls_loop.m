function lv = tm_pls_loop(Xcal, Xtest, lvs)
% ** PLS-DA model for trait mining (FIGS) ** 
% This function will run a series PLS regression to estimate the
% optimal complecity
%
% Syntax: 	lv = tm_pls_loop(Xcal, Xtest, lvs)
%
%   INPUT:
%     Xcal - Calibration set (DSO)
%     Xtest - Test set (DSO)
%     lvs - number of latent variables for the last PLS model
%
%   OUTPUT:
%     Displays on the screen a summary of the classification
%
%   Example: 
%       tm_pls_loop(Xcal, Xtest, 20);
%
% What-To-Look-For:
%     ... todo
%
...
fprintf('\n')
disp('------------------------------------------------');
fprintf(1,'-------- PLS classification (LV 1 to LV %d) ---------\n', lvs);
disp('-----------------------------------------------');
fprintf('\n')
...
% -- Loop LVs for finding PLS model complexity
% clear a; clear loops; 
n=0; 
%loops = 3;
a = zeros(lvs, 12);
...
for i = 1:1:lvs
    n = n+1; 
    a(n, 1) = i;
    [a(n,2), a(n,3), a(n,4), a(n,5), a(n,6), a(n,7), a(n,8), a(n,9), a(n,10), a(n,11), a(n,12), a(n,13)] = tm_pls (Xcal, Xtest, i); 
    % -- a(n,6) = 0.446; % DEBUG
end
...
fprintf('\n');
%fprintf('\t Max Kappa: %6.4f   at LV: %3.0f \n', max(a(:,2)), find(a(:,2) == max(a(:,2))));
%fprintf('\t Max PA   : %6.4f   at LV: %3.0f \n', max(a(:,6)), find(a(:,6) == max(a(:,6))));
%fprintf('\t Max PVV  : %6.4f   at LV: %3.0f \n', max(a(:,7)), find(a(:,7) == max(a(:,7))));
%fprintf('--------------- \n');
